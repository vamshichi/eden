...
const subcategories = ['All', 'Kurtas', 'Shirts', 'Blazers', 'Ethnic Wear', 'Casual Wear', 'Formal Wear']
...
export default function MenPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered
  }, [menProducts, priceRange, selectedSizes, sortBy])
  ...
  const clearFilters = () => { setPriceRange([0, 5000]); setSelectedSizes([]); setSelectedColors([]); setSortBy('featured') }
    ...
    <div className="min-h-screen bg-cream">
      ...
      <main className="pt-32 pb-20">
        <div className="relative h-64 bg-charcoal overflow-hidden">
          <div className="absolute inset-0 bg-cover bg-center opacity-50" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1507680434567-5739c80be1ac?w=1920&h=600&fit=crop)' }} />
          <div className="relative h-full flex items-center justify-center text-center">
            <div>
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-serif text-4xl md:text-5xl text-white mb-3">Men&apos;s Collection</motion.h1>
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-cream/80">{menProducts.length} Products</motion.p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4">
              <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                ...
                <SheetContent side="left" className="w-[300px] bg-cream">
                  ...
                  <FilterContent priceRange={priceRange} setPriceRange={setPriceRange} selectedSizes={selectedSizes} toggleSize={toggleSize} selectedColors={selectedColors} toggleColor={toggleColor} clearFilters={clearFilters} hasActiveFilters={hasActiveFilters} sizes={sizes} colors={colors} />
                </SheetContent>
              </Sheet>
...
function FilterContent({ priceRange, setPriceRange, selectedSizes, toggleSize, selectedColors, toggleColor, clearFilters, hasActiveFilters, sizes, colors }: { priceRange: number[]; setPriceRange: (r: number[]) => void; selectedSizes: string[]; toggleSize: (s: string) => void; selectedColors: string[]; toggleColor: (c: string) => void; clearFilters: () => void; hasActiveFilters: boolean; sizes: string[]; colors: { name: string; hex: string }[] }) {
  return (
    <div className="space-y-6">
      ...
      <div><h3 className="font-medium text-charcoal mb-4">Size</h3><div className="flex flex-wrap gap-2">{sizes.map((size) => (<button key={size} onClick={() => toggleSize(size)} className={`px-3 py-1.5 text-sm border rounded-md transition-colors ${selectedSizes.includes(size) ? 'bg-gold text-white border-gold' : 'border-border text-charcoal hover:border-gold'}`}>{size}</button>))}</div></div>
